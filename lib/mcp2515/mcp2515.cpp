...
MCP2515::ERROR MCP2515::setOneShotMode(const bool enable)
{   
    uint8_t desiredValue = enable ? CANCTRL_OSM : 0;
    modifyRegister(MCP_CANCTRL, CANCTRL_OSM, desiredValue);

    unsigned long endTime = millis() + 10;

    bool modeMatch = false;
    while (millis() < endTime) {
        uint8_t newmode = readRegister(MCP_CANCTRL);

        if ((newmode & CANCTRL_OSM) == desiredValue) {
            modeMatch = true;
            break;
        }
    }
